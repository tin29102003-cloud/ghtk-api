    ...
    {//xclientauthoption là 1 Ioption xong Ioption này lại kế thừa từ AuthenticationSchemeOptions nên nó sẽ có các thuộc tính của AuthenticationSchemeOptions, và nó cũng có thêm thuộc tính ClientSourceValidator để xác thực giá trị của header X-Client-Source
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
                ...
                &&   VerifyClient(clientScourceValue, tokenValue, out var principal))
            {
                ...
                ((ClaimsIdentity)(principal!.Identity!)).AddClaim(new Claim("PartnerId", clientScourceValue));//dòng này là thêm một claim mới vào identity của principal, claim này có tên là "X-Client-Source" và giá trị là clientScourceValue, tức là giá trị của header X-Client-Source. Việc thêm claim này vào identity sẽ giúp chúng ta có thể truy cập được giá trị của header X-Client-Source trong các phần khác của ứng dụng, ví dụ như trong controller để lấy ra giá trị này và sử dụng nó cho các mục đích khác nhau.
